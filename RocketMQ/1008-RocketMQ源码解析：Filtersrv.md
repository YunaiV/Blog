...
* 🦅 一个 `Filtersrv` 对应一个 `Broker`。
* 🦅 一个 `Broker` 可以对应**多个** `Filtersrv`。因此，`Filtersrv` 的**高可用**可以通过启动多个 `Filtersrv` 实现。
...
![Filtersrv过滤类](images/1008/Filtersrv过滤类.jpeg)
...
* 🦅 `Consumer` 拉取消费消息时，若消息 `Topic` 使用过滤类，则会从 `Broker` 对应的 `Filtersrv` 列表随机选择一个。**如果选择不到 `Filtersrv`，则无法拉取消息。因此，`Filtersrv` 一定要做高可用**。
...
* 🦅 `Filtersrv` 实际可以理解成一个 `Consumer`，因此向 `Broker` 拉取消息时，使用的 `DefaultMQPullConsumer` 的方法逻辑。
